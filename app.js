...
(function() {
  ...
  if (!lightbox || !lbImg || projCards.length === 0) return;

  // Open lightbox on card click
  projCards.forEach(card => {
    card.addEventListener('click', (e) => {
      // Prevent opening if they click tags or something else (optional)
  ...
  const closeLb = () => {
    ...
    document.body.style.overflow = '';
    // small delay before resetting src to avoid visual glitch during transition
    setTimeout(() => { if (!lightbox.classList.contains('open')) lbImg.src = ''; }, 300);
  };

  lbClose.addEventListener('click', closeLb);
  lightbox.addEventListener('click', (e) => {
    // If they click outside the image itself, close
...
/* ═══════ 24. CLICK ON IMAGE TOAST POPUP ═══════════ */
(function() {
  const toast = $('#imageToast');
  const worksGrid = $('#worksGrid');
  
  if (!toast || !worksGrid) return;
  
  let toastShown = false;

  const io = new IntersectionObserver(entries => {
    entries.forEach(en => {
      if (en.isIntersecting && !toastShown) {
        toastShown = true;
        // Show the toast
        ...
        io.unobserve(worksGrid);
      }
    });
  }, { threshold: 0.3 }); // Trigger when 30% of the works section is in view

  io.observe(worksGrid);
})();
